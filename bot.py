"""
🤖 AlphaDrop Bot v22.1 — Fresh-Only + NFT Sources + Persistent Dedup
Changes vs v12:
...
ROBINHOOD_SOURCE_PRIORITY = {
    "Robinhood Chain Ecosystem", "Robinhood NFT Marketplace",
    "Robinhood NFT Activity", "OpenSea Robinhood Collections",
    "CoinGecko Robinhood NFTs", "Auron Robinhood Launches",
}
...
def calculate_rating(item):
    ...
    # === INSTANT AIRDROP SIGNAL ===
    instant_signals = [
        "airdrop live", "claim now", "claim live", "claims open",
        "token claim", "distribution live", "claim deadline",
        "check eligibility", "eligibility checker"
    ]
    if t == "instant" and any(x in text for x in instant_signals):
        score += 0.5
...
class DeepFetcher:
    ...
    async def _get(self, url, timeout=15, fmt="json"):
        ...
        # Avoid repeatedly trying hosts that already failed DNS resolution.
    ...
    async def deep_scrape_page(self, url):
        ...
        html = await self._get(url, timeout=12, fmt="text")
        if not html:
            return {}
    ...
    async def enrich_all(self, items):
        print(f"  🔍 Deep scraping {len(items)} items (DNS-safe, concurrency=3)...")
        tasks_list = [self._enrich_one(item) for item in items]
        enriched = await asyncio.gather(*tasks_list, return_exceptions=False)
...
class Fetcher:
    ...
    # === ROBINHOOD CHAIN NFT / DEGEN DISCOVERY ===
    async def rh_official_ecosystem(self):
        text = await self._get("https://robinhood.com/chain/ecosystem?lang=en", 20, "text")
        return self._parse_discovery_html(text, "Robinhood Chain Ecosystem", forced_type="early", max_items=30)

    async def rh_nft_marketplace(self):
        text = await self._get("https://www.robinhoodnfts.com/marketplace/", 20, "text")
        return self._parse_discovery_html(text, "Robinhood NFT Marketplace", forced_type="nft", max_items=40)

    async def rh_nft_activity(self):
        text = await self._get("https://www.robinhoodnfts.com/marketplace/activity.html", 20, "text")
        return self._parse_discovery_html(text, "Robinhood NFT Activity", forced_type="nft", max_items=30)

    async def rh_opensea_collections(self):
        text = await self._get("https://opensea.io/collections/chain/robinhood", 20, "text")
        return self._parse_discovery_html(text, "OpenSea Robinhood Collections", forced_type="nft", max_items=40)

    async def rh_coingecko_nfts(self):
        text = await self._get("https://www.coingecko.com/en/nft/chains/robinhood", 20, "text")
        return self._parse_discovery_html(text, "CoinGecko Robinhood NFTs", forced_type="nft", max_items=40)

    async def rh_auron(self):
        text = await self._get("https://auron.lol/", 20, "text")
        return self._parse_discovery_html(text, "Auron Robinhood Launches", max_items=35)

    # === NEW PROJECT / WAITLIST DISCOVERY ===
    async def wait_productwait_crypto(self):
        text = await self._get("https://productwait.com/c/crypto", 20, "text")
        return self._parse_discovery_html(text, "Product Wait Crypto", forced_type="waitlist", max_items=35)

    async def wait_rootdata_projects(self):
        text = await self._get("https://www.rootdata.com/projects", 20, "text")
        return self._parse_discovery_html(text, "RootData New Projects", forced_type="early", max_items=35)

    async def wait_producthunt_web3(self):
        text = await self._get("https://www.producthunt.com/categories/web3?order=recent_launches", 20, "text")
        return self._parse_discovery_html(text, "Product Hunt Web3", forced_type="waitlist", max_items=30)

    async def wait_producthunt_crypto(self):
        text = await self._get("https://www.producthunt.com/categories/crypto-tools?order=recent_launches", 20, "text")
        return self._parse_discovery_html(text, "Product Hunt Crypto Tools", forced_type="waitlist", max_items=30)

    async def wait_galxe(self):
        text = await self._get("https://galxe.com/", 20, "text")
        return self._parse_discovery_html(text, "Galxe", forced_type="waitlist", max_items=40)

    async def wait_zealy(self):
        text = await self._get("https://zealy.io/", 20, "text")
        return self._parse_discovery_html(text, "Zealy", forced_type="waitlist", max_items=40)
    ...
    # === PARSERS ===
    def _parse_discovery_html(self, text, source, forced_type=None, max_items=25):
        """Lightweight HTML discovery parser for project/NFT/waitlist directories."""
        if not text:
            return []
        items = []
        try:
            raw = html_lib.unescape(text)
            # Extract simple anchor title/link pairs without requiring BeautifulSoup.
            anchors = re.findall(
                r'<a\b[^>]*href=["\']([^"\']+)["\'][^>]*>(.*?)</a>',
                raw, flags=re.I | re.S
            )
            signals = [
                "nft", "mint", "collection", "drop", "free mint", "freemint",
                "waitlist", "wait list", "early access", "priority access",
                "launch", "upcoming", "coming soon", "prelaunch", "beta",
                "whitelist", "allowlist", "airdrop", "points", "quest",
                "campaign", "web3", "crypto", "onchain", "genesis",
                "membership", "pass", "founding"
            ]
            seen = set()
            for href, label_html in anchors:
                label = strip_html(label_html).strip()
                if not label or len(label) < 3 or len(label) > 180:
                    continue
                ...
                if href.startswith(("http://", "https://")):
                    ...
                    # Use the source page's hostname when resolving relative links.
                    base_by_source = {
                        "Robinhood Chain Ecosystem": "https://robinhood.com",
                        "Robinhood NFT Marketplace": "https://www.robinhoodnfts.com",
                        "Robinhood NFT Activity": "https://www.robinhoodnfts.com",
                        "OpenSea Robinhood Collections": "https://opensea.io",
                        "CoinGecko Robinhood NFTs": "https://www.coingecko.com",
                        "Auron Robinhood Launches": "https://auron.lol",
                        "Product Wait Crypto": "https://productwait.com",
                        "RootData New Projects": "https://www.rootdata.com",
                        "Product Hunt Web3": "https://www.producthunt.com",
                        "Product Hunt Crypto Tools": "https://www.producthunt.com",
                        "Galxe": "https://galxe.com",
                        "Zealy": "https://zealy.io",
                    }
                    link = urljoin(base_by_source.get(source, ""), href)
                else:
                    continue
                key = (label.lower(), link.lower())
                if key in seen:
                    continue
                seen.add(key)
                item_type = forced_type or detect_type(combined)
                items.append({
                    "name": label[:100],
                    "desc": f"{source} discovery | {label[:240]}",
                    "link": link,
                    "source": source,
                    "type": item_type,
                    "hype": self._hype_score(combined),
                    "chain": "Robinhood Chain" if any(x in combined for x in [
                        "robinhood", "rh chain", "hoodies", "hood", "robin"
                    ]) else extract_chain(combined),
                    "deadline": extract_deadline(label) or "TBA",
            ...
            # Always retain a page-level discovery signal when the page itself is
            # relevant but its links don't expose useful titles.
            if not items:
                body = strip_html(raw)
                low = body.lower()
                if any(s in low for s in signals):
                    item_type = forced_type or detect_type(low)
                    items.append({
                        "name": f"{source} discovery",
                        "desc": body[:280],
                        "link": "",
                        "source": source,
                        "type": item_type,
                        "hype": self._hype_score(low),
                        "chain": "Robinhood Chain" if "robinhood" in low else extract_chain(low),
                        "deadline": extract_deadline(body) or "TBA",
                        "difficulty": detect_difficulty(low),
                        "funding": "N/A",
                        "published_at": datetime.now().isoformat(),
                        "deep": {}
                    })
    ...
    def _parse_general_rss(self, text, source):
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                signals = [
                    "testnet", "devnet", "mainnet", "launch", "launched", "launching",
                    "early access", "early user", "waitlist", "waiting list",
                    "whitelist", "allowlist", "airdrop", "airdrop live",
                    "claim now", "claim live", "claims open", "token claim",
                    "claim deadline", "distribution", "distribution live",
                    "token distribution", "eligible", "eligibility", "check eligibility",
                    "eligibility checker", "checker", "snapshot", "snapshot taken",
                    "points", "points program", "points campaign", "rewards",
                    "quests", "quest campaign", "campaign", "incentivized",
                    "incentive", "funding", "raised", "seed round", "series a",
                    "series b", "grant", "grants", "builder program", "ecosystem program",
                    "hackathon", "beta", "closed beta", "open beta", "public beta",
                    "sale", "public sale", "private sale", "token generation",
                    "token generation event", "tge", "listing", "token unlock",
                    "nft", "nft launch", "nft mint", "free mint", "freemint",
                    "mint pass", "collection", "digital collectible", "sbt",
                    "holder rewards", "token gated", "creator drop", "art drop",
                    "genesis pass", "membership pass", "founding users",
                    "founding members", "genesis access", "prelaunch", "pre-launch"
                ]
                if not any(s in combined for s in signals):
                    continue
    ...
    async def rss_kucoin(self):
        text = await self._get("https://www.kucoin.com/rss/news", 20, "text")
        return self._parse_nft_rss(text, "KuCoin") if "KuCoin" in ("Zora", "Immutable") else self._parse_general_rss(text, "KuCoin")

    async def rss_okx(self):
        text = await self._get("https://www.okx.com/feed", 20, "text")
        return self._parse_nft_rss(text, "OKX") if "OKX" in ("Zora", "Immutable") else self._parse_general_rss(text, "OKX")

    async def rss_binance(self):
        text = await self._get("https://www.binance.com/en/support/announcement/rss", 20, "text")
        return self._parse_nft_rss(text, "Binance") if "Binance" in ("Zora", "Immutable") else self._parse_general_rss(text, "Binance")

    async def rss_gate(self):
        text = await self._get("https://www.gate.com/rss", 20, "text")
        return self._parse_nft_rss(text, "Gate") if "Gate" in ("Zora", "Immutable") else self._parse_general_rss(text, "Gate")

    async def rss_mexc(self):
        text = await self._get("https://www.mexc.com/support/articles/rss", 20, "text")
        return self._parse_nft_rss(text, "MEXC") if "MEXC" in ("Zora", "Immutable") else self._parse_general_rss(text, "MEXC")

    async def rss_aptos(self):
        text = await self._get("https://aptosfoundation.org/currents/feed", 20, "text")
        return self._parse_nft_rss(text, "Aptos") if "Aptos" in ("Zora", "Immutable") else self._parse_general_rss(text, "Aptos")

    async def rss_sui(self):
        text = await self._get("https://blog.sui.io/rss/", 20, "text")
        return self._parse_nft_rss(text, "Sui") if "Sui" in ("Zora", "Immutable") else self._parse_general_rss(text, "Sui")

    async def rss_avalanche(self):
        text = await self._get("https://www.avax.network/blog/rss.xml", 20, "text")
        return self._parse_nft_rss(text, "Avalanche") if "Avalanche" in ("Zora", "Immutable") else self._parse_general_rss(text, "Avalanche")

    async def rss_polygon(self):
        text = await self._get("https://polygon.technology/blog/rss.xml", 20, "text")
        return self._parse_nft_rss(text, "Polygon") if "Polygon" in ("Zora", "Immutable") else self._parse_general_rss(text, "Polygon")

    async def rss_immutable(self):
        text = await self._get("https://www.immutable.com/blog/rss.xml", 20, "text")
        return self._parse_nft_rss(text, "Immutable") if "Immutable" in ("Zora", "Immutable") else self._parse_general_rss(text, "Immutable")

    async def rss_zora(self):
        text = await self._get("https://zora.co/blog/rss.xml", 20, "text")
        return self._parse_nft_rss(text, "Zora") if "Zora" in ("Zora", "Immutable") else self._parse_general_rss(text, "Zora")

    async def rss_mirror(self):
        text = await self._get("https://mirror.xyz/feed", 20, "text")
        return self._parse_nft_rss(text, "Mirror") if "Mirror" in ("Zora", "Immutable") else self._parse_general_rss(text, "Mirror")

    async def rss_defillama(self):
        text = await self._get("https://defillama.com/rss.xml", 20, "text")
        return self._parse_nft_rss(text, "DefiLlama") if "DefiLlama" in ("Zora", "Immutable") else self._parse_general_rss(text, "DefiLlama")
    ...
    def _source_map(self):
        return {
            ...
            "Robinhood Chain Ecosystem": self._safe_source('rh_official_ecosystem'),
            "Robinhood NFT Marketplace": self._safe_source('rh_nft_marketplace'),
            "Robinhood NFT Activity": self._safe_source('rh_nft_activity'),
            "OpenSea Robinhood Collections": self._safe_source('rh_opensea_collections'),
            "CoinGecko Robinhood NFTs": self._safe_source('rh_coingecko_nfts'),
            "Auron Robinhood Launches": self._safe_source('rh_auron'),
            "Product Wait Crypto": self._safe_source('wait_productwait_crypto'),
            "RootData New Projects": self._safe_source('wait_rootdata_projects'),
            "Product Hunt Web3": self._safe_source('wait_producthunt_web3'),
            "Product Hunt Crypto Tools": self._safe_source('wait_producthunt_crypto'),
            "Galxe": self._safe_source('wait_galxe'),
            "Zealy": self._safe_source('wait_zealy'),
    def _merge_source_consensus(self, items):
        """Deduplicate and attach multi-source consensus before scoring/filtering."""
        groups = {}
        for item in items:
            name = re.sub(r"[^a-z0-9]+", " ", str(item.get("name", "")).lower()).strip()
            link = str(item.get("link", "")).lower().split("#")[0].rstrip("/")
            key = link if link else name
            if not key:
                continue
        ...
        for key, group in groups.items():
            ...
            base["multi_source_confirmed"] = len(sources) >= 2
            base["robinhood_priority"] = any(s in ROBINHOOD_SOURCE_PRIORITY for s in sources)

            # Keep scoring balanced: consensus is information quality, not a
            # large score multiplier.
            if len(sources) >= 3:
                base["confidence"] = "high"
            elif len(sources) == 2:
                base["confidence"] = "medium"
            else:
                base["confidence"] = "single-source"

            merged.append(base)

        return merged

            "CoinMarketCap": self.rss_coinmarketcap(),
            "KuCoin": self.rss_kucoin(),
            "OKX": self.rss_okx(),
            "Binance": self.rss_binance(),
            "Gate": self.rss_gate(),
            "MEXC": self.rss_mexc(),
            "Aptos": self.rss_aptos(),
            "Sui": self.rss_sui(),
            "Avalanche": self.rss_avalanche(),
            "Polygon": self.rss_polygon(),
            "Immutable": self.rss_immutable(),
            "Zora": self.rss_zora(),
            "Mirror": self.rss_mirror(),
            "DefiLlama": self.rss_defillama(),
...
class UI:
    ...
    def help_menu(cls):
        embed = discord.Embed(
            title="🤖 AlphaDrop Bot v22.1 — Fresh-Only, Expanded NFT Sources",
            description=(
