# V24 — Less restrictive potential filtering
# Discovery is broad; potential score ranks results instead of hard-filtering them.
# Obvious unrelated content is still filtered.

"""
🤖 AlphaDrop Bot v24.0 — Fresh-Only + NFT Sources + Persistent Dedup
Changes vs v12:
...
NFT_ACTIONABLE_SIGNALS = [
    "free mint", "freemint", "gtd", "guaranteed whitelist",
    "guaranteed spot", "fcfs", "first come first served",
    "mint date", "mint time", "mint price", "minting now",
    "mint starts", "public mint", "private mint", "presale",
    "whitelist", "allowlist", "mintlist", "wl spot", "wl spots",
    "free claim", "nft claim", "raffle", "giveaway",
    "upcoming mint", "upcoming drop", "nft drop", "collection drop",
    "launchpad", "drop calendar", "mint calendar", "gtd spot",
]

NFT_NEWS_ONLY_SIGNALS = [
    "market analysis", "price analysis", "floor price analysis",
    "market update", "industry report", "regulation", "regulatory",
    "sec", "lawsuit", "court", "fundraising news", "interview",
    "weekly roundup", "news roundup", "opinion", "explained",
]

WAITLIST_CATEGORY_SIGNALS = {
    "airdrop": [
        "airdrop waitlist", "token waitlist", "waitlist airdrop",
        "join waitlist", "early access", "priority access",
        "eligibility", "claim waitlist", "token launch waitlist",
    ],
    "nft": [
        "nft waitlist", "mint waitlist", "whitelist", "allowlist",
        "mintlist", "gtd", "fcfs", "free mint", "collection",
        "nft drop", "mint pass",
    ],
    "event": [
        "event waitlist", "conference", "summit", "meetup",
        "hackathon", "workshop", "side event", "registration",
        "rsvp", "require approval", "join waitlist", "limited capacity",
        "event pass",
    ],
}

def is_actionable_nft(text):
    t = text.lower()
    hits = sum(1 for x in NFT_ACTIONABLE_SIGNALS if x in t)
    news_hits = sum(1 for x in NFT_NEWS_ONLY_SIGNALS if x in t)
    # A single strong mint signal is enough; generic "NFT" alone is not.
    return hits >= 1 and not (news_hits >= 2 and hits < 2)

def detect_waitlist_category(text):
    t = text.lower()
    scores = {k: sum(1 for s in vals if s in t) for k, vals in WAITLIST_CATEGORY_SIGNALS.items()}
    best = max(scores, key=scores.get)
    return best if scores[best] > 0 else "general"

def detect_type(text):
    t = text.lower()
    if is_actionable_nft(t):
        return "nft"
...
class Fetcher:
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                items.append({
                    ...
                    "source": source,
                    "type": "waitlist",
                    "waitlist_category": detect_waitlist_category(combined),
                    "hype": self._hype_score(combined),
                    "chain": extract_chain(combined),
        ...
        try:
            ...
            for item in root.findall(".//item"):
                ...
                signals = [
                    ...
                    "waitlist", "whitelist", "allowlist", "airdrop", "airdrop live",
                    ...
                    "checker", "snapshot", "points", "rewards", "campaign",
                    "incentivized", "funding", "raised", "nft", "mint", "collection",
                    "ecosystem", "beta", "sale"
                ]
                ...
                items.append({
                    ...
                    "waitlist_category": detect_waitlist_category(combined) if detect_type(combined) == "waitlist" else None,
                    "hype": self._hype_score(combined),
                    "chain": extract_chain(combined),
    ...
    def _parse_discovery_html(self, html, source, mode="mixed", max_items=30):
        """Extract actionable project/drop/event links from public discovery pages."""
        if not html:
            return []
        items = []
        try:
            # Lightweight parser: no extra dependency required.
            pattern = re.compile(
                r"<a[^>]+href=[\"']([^\"']+)[\"'][^>]*>(.*?)</a>",
                re.I | re.S
            )
            ...
            for href, raw_label in pattern.findall(html):
                label = strip_html(raw_label).strip()
                if not label or len(label) < 4 or len(label) > 180:
                    continue
                combined = label.lower()
                if mode == "nft":
                    if not is_actionable_nft(combined):
                        continue
                    item_type = "nft"
                elif mode == "waitlist":
                    if not any(k in combined for vals in WAITLIST_CATEGORY_SIGNALS.values() for k in vals):
                        continue
                    item_type = "waitlist"
                elif mode == "event":
                    if not any(k in combined for k in WAITLIST_CATEGORY_SIGNALS["event"]):
                        continue
                    item_type = "waitlist"
                else:
                    if not (is_actionable_nft(combined) or
                            any(k in combined for vals in WAITLIST_CATEGORY_SIGNALS.values() for k in vals)):
                        continue
                    item_type = detect_type(combined)

                if href.startswith("http"):
                    link = href
                elif href.startswith("/"):
                    bases = {
                        "LuckyGo NFT GTD/FCFS": "https://luckygo.io/",
                        "NFTDropList": "https://www.nftdroplist.com/",
                        "NFTCalendar Live/Upcoming Mints": "https://nftcalendar.io/",
                        "NFTCalendar Upcoming Drops": "https://nftcalendar.io/",
                        "OpenSea Live/Upcoming Drops": "https://opensea.io/",
                        "Luma Crypto Events": "https://luma.com/",
                        "CoinMarketCap Events": "https://coinmarketcap.com/",
                        "Coindar Crypto Events": "https://coindar.org/",
                        "EventMates Web3 Events": "https://eventmates.app/",
                        "QuestN Airdrop/Quest": "https://www.questn.com/",
                        "SourceChain Quest": "https://sourcechain.org/",
                        "Galxe Quest": "https://galxe.com/",
                    }
                    link = urljoin(bases.get(source, ""), href)
                else:
                    continue

                key = (label.lower(), href.split("?")[0])
                if key in seen:
                    continue
                seen.add(key)
                ...
                items.append({
                    ...
                    "source": source,
                    "type": item_type,
                    "waitlist_category": detect_waitlist_category(combined) if item_type == "waitlist" else None,
                    "hype": self._hype_score(combined),
                    "chain": "Robinhood Chain" if any(x in combined for x in [
                        "robinhood chain", "robinhood", "hoodies", "hoodkatz"
    ...
    # === ACTIONABLE NFT DROP / GTD / FCFS SOURCES ===
    async def nft_luckygo(self):
        text = await self._get("https://luckygo.io/", 20, "text")
        return self._parse_discovery_html(text, "LuckyGo NFT GTD/FCFS", "nft", 40)

    async def nft_droplist(self):
        text = await self._get("https://www.nftdroplist.com/", 20, "text")
        return self._parse_discovery_html(text, "NFTDropList", "nft", 30)

    async def nft_calendar_mints(self):
        text = await self._get("https://nftcalendar.io/mints/", 20, "text")
        return self._parse_discovery_html(text, "NFTCalendar Live/Upcoming Mints", "nft", 30)

    async def nft_calendar_events(self):
        text = await self._get("https://nftcalendar.io/events/", 20, "text")
        return self._parse_discovery_html(text, "NFTCalendar Upcoming Drops", "nft", 30)

    async def nft_opensea_drops(self):
        text = await self._get("https://opensea.io/drops", 20, "text")
        return self._parse_discovery_html(text, "OpenSea Live/Upcoming Drops", "nft", 30)

    # === WAITLIST / NEW PROJECT SOURCES ===
    async def wait_luma_crypto(self):
        text = await self._get("https://luma.com/crypto", 20, "text")
        return self._parse_discovery_html(text, "Luma Crypto Events", "event", 35)

    async def wait_cmc_events(self):
        text = await self._get("https://coinmarketcap.com/events/", 20, "text")
        return self._parse_discovery_html(text, "CoinMarketCap Events", "event", 35)

    async def wait_coindar(self):
        text = await self._get("https://coindar.org/", 20, "text")
        return self._parse_discovery_html(text, "Coindar Crypto Events", "event", 35)

    async def wait_eventmates(self):
        text = await self._get("https://eventmates.app/", 20, "text")
        return self._parse_discovery_html(text, "EventMates Web3 Events", "event", 30)

    async def wait_questn(self):
        text = await self._get("https://www.questn.com/", 20, "text")
        return self._parse_discovery_html(text, "QuestN Airdrop/Quest", "waitlist", 35)

    async def wait_sourcechain(self):
        text = await self._get("https://sourcechain.org/events/quest/", 20, "text")
        return self._parse_discovery_html(text, "SourceChain Quest", "waitlist", 35)

    async def wait_galxe_quest(self):
        text = await self._get("https://galxe.com/quest", 20, "text")
        return self._parse_discovery_html(text, "Galxe Quest", "waitlist", 35)
    ...
    def _source_map(self):
        return {
            ...
            "LuckyGo NFT GTD/FCFS": self._safe_source('nft_luckygo'),
            "NFTDropList": self._safe_source('nft_droplist'),
            "NFTCalendar Live/Upcoming Mints": self._safe_source('nft_calendar_mints'),
            "NFTCalendar Upcoming Drops": self._safe_source('nft_calendar_events'),
            "OpenSea Live/Upcoming Drops": self._safe_source('nft_opensea_drops'),

            "Luma Crypto Events": self._safe_source('wait_luma_crypto'),
            "CoinMarketCap Events": self._safe_source('wait_cmc_events'),
            "Coindar Crypto Events": self._safe_source('wait_coindar'),
            "EventMates Web3 Events": self._safe_source('wait_eventmates'),
            "QuestN Airdrop/Quest": self._safe_source('wait_questn'),
            "SourceChain Quest": self._safe_source('wait_sourcechain'),
            "Galxe Quest": self._safe_source('wait_galxe_quest'),
        }
    ...
    def _merge_source_consensus(self, items):
        ...
        for item in items:
            ...
            key = link if link.startswith("http") else name
            if key:
    ...
    async def fetch_all(self):
        ...
        raw_count = len(flat)
        flat = self._merge_source_consensus(flat)

        print(f"\n🔎 Filtered {raw_count} raw items → {len(flat)} unique actionable items")
        print(f"   • Multi-source confirmed: {sum(1 for x in flat if x.get('multi_source_confirmed'))}")
        print(f"   • NFT drops only: {sum(1 for x in flat if x.get('type') == 'nft')}")
        print(f"   • Waitlist: {sum(1 for x in flat if x.get('type') == 'waitlist')}")
        print(f"   • Robinhood mentions: {sum(1 for x in flat if 'robinhood' in str(x.get('chain','')).lower())}")

        print(f"\n🔍 Deep scraping {len(flat)} filtered items...")
        ...
        return {"Filtered / Multi-Source": enriched}
...
class UI:
    ...
    def help_menu(cls):
        embed = discord.Embed(
            title="🤖 AlphaDrop Bot v24.0 — Fresh-Only, Expanded NFT Sources",
            description=(
